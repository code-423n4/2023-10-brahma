...
contract ConsoleOpBuilder_EnableExecutorsOnSubaccountTest is Test, ConsoleFactory("offchain/addressManager.ts") {
    ...
    function testEnableExecutorMulticall(uint160 numberOfExecutors) public {
        ...
        assertTrue(IGnosisSafe(subaccount).isModuleEnabled(address(executorPlugin)), "Executor plugin not enabled");

        for (uint160 i = 1; i <= numberOfExecutors; i++) {
            assertTrue(executorRegistry.isExecutor(subaccount, _executors[i - 1]), "Executor not found");
        }
    }
