...
contract SafeModerator_CheckTransactionTest is
    Test,
    ModSignature,
    ...
    MockTransactionValidator
{
    ...
    function setUp() public {
        ...
        safe = IGnosisSafe(safeDeployer.deploySubAccount(owners, 1, commit, keccak256("salt")));
    }

    function validatePreTransaction(TransactionValidator.SafeTransactionParams memory) external view override {
        if (isTxValid) return;
    ...
    function testDeploy() public {
        ...
        assertEq(SafeHelper._getGuard(address(safe)), address(safeModerator));
    }
